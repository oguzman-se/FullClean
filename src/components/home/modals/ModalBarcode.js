...
function ModalBarcode(props) {
  ...
  const {products, currentProducto, setCurrentProducto, AllCodigos, setAllCodigos} = useHome()
  const {showBarcode, setShowBarcode, barcode} = props;
  const submit = async () => {
    const exist = AllCodigos.find(x=> x.codigo === barcode.toLowerCase())
    if(exist === undefined){
  ...
  return (
    <>
      ...
      >
        ...
        <Modal.Body>
        <h3>El codigo de barra que ha ingresado no existe ¿Desea crearlo?
        </h3>
        <select class="form-select form-control custom-input" aria-label="Default select example"
         onChange={(e) => handleChange(e)} name="producto_id">
