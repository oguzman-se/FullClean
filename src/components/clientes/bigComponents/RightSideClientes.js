...
const {facturasXcliente} = useHome()

const filtroBuscador = (item) => {
    console.log("tenemos este pedidooo", item);
    const validText = (it) => {
        let validator = false;
        if(facturasXcliente && facturasXcliente[0]?.estado){
    ...
    if (
        ...
        validDay(item) &&
        facturasXcliente && 
        facturasXcliente[0]?.estado
    ){return item;}else if(validText(item)){
        return item;
    }
